...
describe('Checks if API archiveNotification request is called when archiveNotification method is called', () => {
  test('archiveNotification patch request is 200', async () => {
    ...
    wrapper.setProps({archivedSelected: false});
...
describe('check starring api call is correct',  () => {
...
describe('check deleting api call is correct',  () => {
    ...
    test('check emitted correctly', async () =>  {
        ...
        expect(wrapper.emitted().deleteNotification[0]).toEqual([1])
    });
...
describe('Checks if API un-archive Notification request is called when un-archive Notification method is called', () => {
  test('archiveNotification patch request is 200', async () => {
    const response = {
      response: {status: 200}
    }
    wrapper.setProps({archivedSelected: true});
    wrapper.vm.updatedNotification.id = 1;
    await Api.patchNotification.mockResolvedValue(response);
    await wrapper.vm.archiveNotification();
    expect(Api.patchNotification).toHaveBeenCalledWith(1, {"archived": false});
  })
});
